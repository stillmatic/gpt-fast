...
def sample(
    logits,
    should_compute_entropy: bool = False,
    temperature: float = 1.0,
    top_k: Optional[int] = None,
    min_p: Optional[float] = None,
):
    ...
    for i in range(num_new_tokens):
        with torch.backends.cuda.sdp_kernel(
            ...
            next_token, next_prob, entropy, _ = decode_one_token(
                ...
                cur_token,
                input_pos,
                should_compute_entropy=should_compute_entropy,
                **sampling_kwargs,
            )
            input_pos += 1
...
def generate(
    ...
    **sampling_kwargs,
    ...
    draft_accept_pairs = defaultdict(int)
    ...
    if is_speculative:
        ...
        while input_pos < T_new - 1:
            ...
            draft_accept_pairs[(len(next_tokens) - 1, draft_count)] += 1
    ...
    generate_stats = {
        ...
        "draft_accept_pairs": draft_accept_pairs,
    }
...
def _aggregate_and_visualize_pairs(draft_accept_pairs):
    """
    Creates a matrix visualization from pre-aggregated draft and accept counts.

    Args:
        data: draft_accept_pairs: List of List of tuples (draft_count, accept_count).

    Returns:
        matrix: 2D list representing the frequency matrix
        row_labels: List of draft counts
        col_labels: List of accept counts
    """
    # First, aggregate all pairs into a defaultdict
    pair_counts = defaultdict(int)
    for row in draft_accept_pairs:
        for accept, draft in row:
            pair_counts[(accept, draft)] += 1

    # Get the maximum dimensions needed
    max_accept = max(accept for accept, _ in pair_counts.keys()) + 1
    max_draft = max(draft for _, draft in pair_counts.keys()) + 1

    # Create and fill the matrix
    matrix = [[0 for _ in range(max_accept)] for _ in range(max_draft)]
    for (accept, draft), count in pair_counts.items():
        matrix[draft][accept] = count

    return matrix, list(range(max_draft)), list(range(max_accept))
...
{user_content}<|eot_id|><|start_header_id|>assistant<|end_header_id|>"""
    ...
    for i in range(start, num_samples):
        ...
        with prof:
            y, metrics = generate(
                ...
                speculate_min_k=speculate_min_k,
            ...
            aggregate_metrics["draft_accept_pairs"].append(
                metrics["draft_accept_pairs"]
            )
        if i == -1:
    ...
    if is_speculative:
        ...
        draft_accept_pairs = _aggregate_and_visualize_pairs(
            aggregate_metrics["draft_accept_pairs"]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Maximum speculative execution depth.",
    )
    parser.add_argument(
        "--speculate_entropy_threshold",
        type=float,
        default=float("inf"),
        help="Maximum entropy to decode another draft token with.",
    )
    parser.add_argument(
        "--speculate_min_k",
        type=int,
        default=0,
        help="Minimum speculative execution depth.",
    )
    parser.add_argument(
    ...
    main(
        ...
        args.device,
        args.min_p,
        args.speculate_entropy_threshold,
